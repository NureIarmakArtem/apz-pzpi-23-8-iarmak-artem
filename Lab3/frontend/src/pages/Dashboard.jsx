...
const Dashboard = () => {
  ...
  // 1. Цей useEffect підтягує статус курьера з бази при завантаженні сторінки
  useEffect(() => {
  ...
  // 2. ОСЬ ЦЬОГО ХУКА НЕ ВИСТАЧАЛО! Він завантажує замовлення щоразу, коли курьер стає Онлайн
  useEffect(() => {
    if (isOnline) {
      fetchOrders();
    } else {
  ...
  const handleCompleteOrder = async (orderId) => {
    try {
      ...
      fetchOrders(); // Оновлюємо список після завершення
    } catch (error) {
