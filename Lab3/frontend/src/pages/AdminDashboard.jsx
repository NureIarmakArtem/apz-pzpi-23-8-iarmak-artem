...
const AdminDashboard = () => {
  ...
  const fetchCouriers = async () => {
    try { 
      ...
      // ОТКРОЙ КОНСОЛЬ (F12) И ПОСМОТРИ СЮДА, ЧТОБЫ УЗНАТЬ ИМЯ ПОЛЯ ТЕЛЕФОНА:
  ...
  const handleAssignOrder = async (e) => {
    ...
    try {
      ...
      fetchOrders();   // Оновлюємо замовлення
      fetchCouriers(); // ДОДАЙ СЮДИ: миттєво оновить статус курьєра в таблиці на BUSY!
